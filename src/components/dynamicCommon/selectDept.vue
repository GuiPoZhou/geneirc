<template>
  <el-dialog title="单位选择" :visible.sync="visible" append-to-body width="800px"
    ...
    <el-form ref="queryForm" :model="queryParams" label-width="80px">
      ...
      <el-table v-loading="loading" :data="dynamicList" >
        ...
        <el-table-column prop="legalPerson" label="单位法人" width="180" />
        <el-table-column prop="creditCode" label="统一社会信用代码" width="180" />
        <el-table-column label="操作" align="center" class-name="right">
          <template slot-scope="scope">
            <el-button @click="selectDeptHandleClick(scope.row)" type="primary" round size="mini">选择</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination
...
export default {
  ...
  methods:{
    ...
    getList() {
      this.loading = true;
      getDeptInfoList(this.queryParams).then(response => {
    ...
    //选择按钮操作
    selectDeptHandleClick(row) {
      this.$emit("selectDeptHandleClick", row);
      this.close();
    },
    open() {
      this.getList();
