...
<!--@select="parseSelectedItems" @select-all="parseSelectedAllItems"      -->
    <el-table :data="itemTableData" border v-loading="loading" element-loading-text="拼命加载中"
              element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)"
              style="width: 100%" height="330" ref="itemTable" :row-style="{ height: '20px' }"
              :cell-style="{ padding: '2px' }" @selection-change="selectionChange"
              :row-key="getRowKeys" :tree-props="{children: 'voListZ'}">
      <el-table-column type="selection" width="50" :selectable="checkSelect" :reserve-selection="true">
      </el-table-column>
      <el-table-column
        prop="itemName"
        label="检测项目名称"
  ...
  export default {
    ...
    methods: {
      ...
      async initItemList(detectType, recordType) {
        ...
        // this.queryParams.detectCategory = this.microStore.state.BeiYuan.categoriesId ? this.microStore.state.BeiYuan.categoriesId : detectType;
        this.$nextTick(() => {
