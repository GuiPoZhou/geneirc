<template>
	<div style="flex:1">
		...
		<el-table :data.sync="extendedProperties.calibrationTurbidityList" @selection-change="handleSelectionChange" ref="multipleTable">
		...
		<el-dialog :title="'标准记录'" :visible.sync="saveFormShow"
			...
			<el-form ref="form" :model="saveForm" :disabled="extendedProperties.formShowType !== 0" size="small" label-width="200px">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="标准液浓度(NTU)" prop="liquidConcentration">
							<el-input v-model="saveForm.liquidConcentration" placeholder="标准液浓度" @input="calcValue1()"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="校正测定值(NTU)" prop="value">
							<el-input v-model="saveForm.value" placeholder="校正测定值" @input="calcValue2()"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="偏差范围(NTU)" prop="deviationRange">
							<el-input v-model="saveForm.deviationRange" placeholder="偏差范围"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="相对误差(NTU)" prop="relativeError">
...
<script>
  ...
  import multipleEditForm from '@/components/recordSheet/multipleColumn/editForm';
    ...
    components: {
			...
			submit() {
				if (this.enterBatchContent != '') {
					this.selectedRowKeys.forEach((data) => {
						this.extendedProperties.calibrationTurbidityList[data][this.enterBatchColumn] = this.enterBatchContent;
					});
				}
				this.cancel();
			},
          ...
          for(let key in this.saveForm){
			...
			handleDelete(record, index) {
				this.$confirm('确认删除该标准信息?', "警告", {
					...
					this.extendedProperties.calibrationTurbidityList.splice(index, 1);
				});
			},
			...
			// 确定选择
			equipmentSubmit() {
				console.log('equipmentSubmit', this.extendedProperties.calibrationTurbidityList)
				if(this.extendedProperties.calibrationTurbidityList){
					console.log('equipmentSubmit push');
					this.extendedProperties.calibrationTurbidityList.push(...this.equipmentChooseDataList);
				}else{
					console.log('equipmentSubmit set');
					this.$set(this.extendedProperties, 'calibrationTurbidityList', JSON.parse(JSON.stringify(this.equipmentChooseDataList)))
				}
			...
			copyRow(index){
				let tempRow = JSON.parse(JSON.stringify(this.extendedProperties.calibrationTurbidityList[index]));
				this.extendedProperties.calibrationTurbidityList.push(tempRow);
			},
			calcValue1(){
				let deviationRange = this.saveForm.liquidConcentration * 0.05
			...
			calcValue2(){
				let relativeError = (this.saveForm.value - this.saveForm.liquidConcentration) / this.saveForm.liquidConcentration * 100
      ...
      editSubmitForm(data){
        data.forEach(item => {
          let deviationRange = item.liquidConcentration * 0.05
          if(!isNaN(deviationRange)){
            item.deviationRange = deviationRange.toFixed(0);
          }
          let relativeError = (item.value - item.liquidConcentration) / item.liquidConcentration * 100
          if(!isNaN(relativeError)){
