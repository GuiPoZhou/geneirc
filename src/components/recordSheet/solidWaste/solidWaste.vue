<template>
	<div style="flex:1">
		...
		<el-table :data.sync="extendedProperties.solidWasteList" @selection-change="handleSelectionChange" ref="multipleTable">
          ...
          if(list && list.length > 0){
            list.forEach(item =>{
              ...
              item.storageConditions = item.preservatives;
					...
					this.$set(this.extendedProperties, 'solidWasteList', list);
				}
			},
			refill() {
			},
			...
			submit() {
				if (this.enterBatchContent != '') {
					this.selectedRowKeys.forEach((data) => {
						this.extendedProperties.solidWasteList[data][this.enterBatchColumn] = this.enterBatchContent;
					});
				}
				this.cancel();
			},
