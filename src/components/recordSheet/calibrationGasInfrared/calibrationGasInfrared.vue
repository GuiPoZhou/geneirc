<template>
	<div style="flex:1">
		...
		<el-table :data.sync="extendedProperties.calibrationGasInfraredList" @selection-change="handleSelectionChange" ref="multipleTable">
				...
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="标准气浓度" prop="gasConcentration">
							<el-input v-model="saveForm.gasConcentration" placeholder="标准气浓度" @input="calcValue()"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="校正测定值1" prop="value1">
							<el-input v-model="saveForm.value1" placeholder="校正测定值1"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="校正测定值2" prop="value2">
							<el-input v-model="saveForm.value2" placeholder="校正测定值2"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="校正测定值3" prop="value3">
							<el-input v-model="saveForm.value3" placeholder="校正测定值3"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="校正测定均值" prop="valueAvg">
							<el-input v-model="saveForm.valueAvg" placeholder="校正测定均值"  @input="calcValue()"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="相对误差（％）" prop="relativeError">
...
<script>
  ...
  import multipleEditForm from '@/components/recordSheet/multipleColumn/editForm';
    ...
    computed: {
			...
			submit() {
				if (this.enterBatchContent != '') {
					this.selectedRowKeys.forEach((data) => {
						this.extendedProperties.calibrationGasInfraredList[data][this.enterBatchColumn] = this.enterBatchContent;
					});
				}
				this.cancel();
			},
          ...
          for(let key in this.saveForm){
			...
			handleDelete(record, index) {
				this.$confirm('确认删除该标准信息?', "警告", {
					...
					this.extendedProperties.calibrationGasInfraredList.splice(index, 1);
				});
			},
			...
			// 确定选择
			equipmentSubmit() {
				console.log('equipmentSubmit', this.extendedProperties.calibrationGasInfraredList)
				if(this.extendedProperties.calibrationGasInfraredList){
					console.log('equipmentSubmit push');
					this.extendedProperties.calibrationGasInfraredList.push(...this.equipmentChooseDataList);
				}else{
					console.log('equipmentSubmit set');
					this.$set(this.extendedProperties, 'calibrationGasInfraredList', JSON.parse(JSON.stringify(this.equipmentChooseDataList)))
				}
			...
			copyRow(index){
				let tempRow = JSON.parse(JSON.stringify(this.extendedProperties.calibrationGasInfraredList[index]));
				this.extendedProperties.calibrationGasInfraredList.push(tempRow);
			},
			calcValue(){
				let relativeError = (this.saveForm.valueAvg - this.saveForm.gasConcentration) / this.saveForm.gasConcentration * 100
				if(!isNaN(relativeError)){
					this.saveForm.relativeError = relativeError.toFixed(1);
				}
			},
      ...
      editSubmitForm(data){
        data.forEach(item => {
          let relativeError = (item.valueAvg - item.gasConcentration) / item.gasConcentration * 100
          if(!isNaN(relativeError)){
            item.relativeError = relativeError.toFixed(1);
          }
        })
      }
		}
