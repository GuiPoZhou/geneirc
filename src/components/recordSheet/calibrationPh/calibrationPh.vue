<template>
	<div style="flex:1">
		...
		<el-table :data.sync="extendedProperties.calibrationPhList" @selection-change="handleSelectionChange" ref="multipleTable">
		...
		<el-dialog :title="'标准记录'" :visible.sync="saveFormShow"
			...
			<el-form ref="form" :model="saveForm" :disabled="extendedProperties.formShowType !== 0" size="small" label-width="200px">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="标准液浓度" prop="liquidConcentration">
							<el-input v-model="saveForm.liquidConcentration" placeholder="标准液浓度" @input="calcValue()"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="校正测定值" prop="value">
							<el-input v-model="saveForm.value" placeholder="校正测定值" @input="calcValue()"/>
						</el-form-item>
					</el-col>
					<el-col :span="24">
...
<script>
  ...
  import multipleEditForm from '@/components/recordSheet/multipleColumn/editForm';
    ...
    computed: {
			...
			submit() {
				if (this.enterBatchContent != '') {
					this.selectedRowKeys.forEach((data) => {
						this.extendedProperties.calibrationPhList[data][this.enterBatchColumn] = this.enterBatchContent;
					});
				}
				this.cancel();
			},
          ...
          for(let key in this.saveForm){
			...
			handleDelete(record, index) {
				this.$confirm('确认删除该标准信息?', "警告", {
					...
					this.extendedProperties.calibrationPhList.splice(index, 1);
				});
			},
			...
			// 确定选择
			equipmentSubmit() {
				console.log('equipmentSubmit', this.extendedProperties.calibrationPhList)
				if(this.extendedProperties.calibrationPhList){
					console.log('equipmentSubmit push');
					this.extendedProperties.calibrationPhList.push(...this.equipmentChooseDataList);
				}else{
					console.log('equipmentSubmit set');
					this.$set(this.extendedProperties, 'calibrationPhList', JSON.parse(JSON.stringify(this.equipmentChooseDataList)))
				}
			...
			copyRow(index){
				let tempRow = JSON.parse(JSON.stringify(this.extendedProperties.calibrationPhList[index]));
				this.extendedProperties.calibrationPhList.push(tempRow);
			},
			calcValue(){
				let relativeError = this.saveForm.value - this.saveForm.liquidConcentration
				if(!isNaN(relativeError)){
					this.saveForm.relativeError = relativeError.toFixed(2);
				}
			},
      ...
      editSubmitForm(data){
        data.forEach(item => {
          let relativeError = item.value - item.liquidConcentration
          if(!isNaN(relativeError)){
            item.relativeError = relativeError.toFixed(2);
          }
        })
      }
		}
