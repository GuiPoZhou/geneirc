<template>
	<div style="flex:1">
		...
		<el-table :data.sync="extendedProperties.calibrationOxidationList" @selection-change="handleSelectionChange" ref="multipleTable">
    ...
    <el-dialog :title="'标准记录'" :visible.sync="saveFormShow"
      ...
      <el-form ref="form" :model="saveForm" :disabled="extendedProperties.formShowType !== 0" size="small" label-width="200px">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="标准液浓度(mV)" prop="liquidConcentration">
              <el-input v-model="saveForm.liquidConcentration" placeholder="标准液浓度" @input="calcValue1()"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="校正测定值1(mV)" prop="value1">
              <el-input v-model="saveForm.value1" placeholder="校正测定值" @input="calcValue2()"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="校正测定值2(mV)" prop="value2">
              <el-input v-model="saveForm.value2" placeholder="校正测定值" @input="calcValue2()"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="校正测定值3(mV)" prop="value3">
              <el-input v-model="saveForm.value3" placeholder="校正测定值" @input="calcValue2()"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="校正测定均值(mV)" prop="valueAvg">
              <el-input v-model="saveForm.valueAvg" placeholder="校正测定值"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="偏差范围(mV)" prop="deviationRange">
              <el-input v-model="saveForm.deviationRange" placeholder="偏差范围"/>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="相对误差(mV)" prop="relativeError">
...
<script>
  ...
  import multipleEditForm from '@/components/recordSheet/multipleColumn/editForm';
    ...
    computed: {
			...
			submit() {
				if (this.enterBatchContent != '') {
					this.selectedRowKeys.forEach((data) => {
						this.extendedProperties.calibrationOxidationList[data][this.enterBatchColumn] = this.enterBatchContent;
					});
				}
				this.cancel();
			},
          ...
          for(let key in this.saveForm){
            ...
            if(!diff){
              this.$set(this.saveForm, key, lastValue);
					...
					this.saveForm = {
						...
						value3: this.selectList[0].value3,
			...
			submitForm(){
				this.selectList.forEach(item => {
					...
					this.$set(item, 'value3', this.saveForm.value3);
			...
			handleDelete(record, index) {
				this.$confirm('确认删除该标准信息?', "警告", {
					...
					this.extendedProperties.calibrationOxidationList.splice(index, 1);
				});
			},
			...
			// 确定选择
			equipmentSubmit() {
				console.log('equipmentSubmit', this.extendedProperties.calibrationOxidationList)
				if(this.extendedProperties.calibrationOxidationList){
					console.log('equipmentSubmit push');
					this.extendedProperties.calibrationOxidationList.push(...this.equipmentChooseDataList);
				}else{
					console.log('equipmentSubmit set');
					this.$set(this.extendedProperties, 'calibrationOxidationList', JSON.parse(JSON.stringify(this.equipmentChooseDataList)))
				}
			...
			copyRow(index){
				let tempRow = JSON.parse(JSON.stringify(this.extendedProperties.calibrationOxidationList[index]));
				this.extendedProperties.calibrationOxidationList.push(tempRow);
			},
			calcValue1(){
			...
			calcValue2(){
				let valueAvg = (this.saveForm.value1 + this.saveForm.value2 + this.saveForm.value3) / 3;
				if(!isNaN(valueAvg)){
      ...
      editSubmitForm(data){
        data.forEach(item => {
          let valueAvg = (item.value1 + item.value2 + item.value3) / 3;
          if(!isNaN(valueAvg)){
