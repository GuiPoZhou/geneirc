<template>
	<div style="flex:1">
		...
		<a-table :columns="tableColumns" rowKey="dataIndex" :scroll="{ x: 2400 }" bordered
				 :data-source="extendedProperties.sampleWaterSurfaceList"
			...
			<template slot="samplingTime" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].samplingTime" :maxLength="maxLength"/>
			</template>

			<template slot="widthTitle">
				河宽(m) <a @click="showAdd('width')" style="color: #1890ff;">批</a>
			</template>
			<template slot="width" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].width" :maxLength="maxLength"/>
			</template>

			<template slot="deepTitle">
				河深(m) <a @click="showAdd('deep')" style="color: #1890ff;">批</a>
			</template>
			<template slot="deep" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].deep" :maxLength="maxLength"/>
			</template>

			<template slot="velocityFlowTitle">
				流速(m/s) <a @click="showAdd('velocityFlow')" style="color: #1890ff;">批</a>
			</template>
			<template slot="velocityFlow" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].velocityFlow" :maxLength="maxLength"/>
			</template>
			...
			<template slot="discharge" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].discharge" :maxLength="maxLength"/>
			</template>
			...
			<template slot="waterTemperature" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].waterTemperature" :maxLength="maxLength"/>
			</template>
			...
			<template slot="airTemperature" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].airTemperature" :maxLength="maxLength"/>
			</template>
			...
			<template slot="atmosphericPressure" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].atmosphericPressure" :maxLength="maxLength"/>
			</template>
			...
			<template slot="windVelocity" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].windVelocity" :maxLength="maxLength"/>
			</template>
			...
			<template slot="windDirection" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].windDirection" :maxLength="maxLength"/>
			</template>
			...
			<template slot="fixative" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].fixative" :maxLength="200"/>
			</template>
			...
			<template slot="preservative" slot-scope="text, record, index">
				<a-input v-model="extendedProperties.sampleWaterSurfaceList[index].preservative" :maxLength="200"/>
			</template>
...
<script>
	const tableColumns = [
		...
		{
			dataIndex: 'velocityFlow',
			align: 'center',
			slots: {title: 'velocityFlowTitle'},
			scopedSlots: {customRender: 'velocityFlow'}
		},
		{
			dataIndex: 'discharge',
			align: 'center',
	...
	export default {
		...
		props: {
			extendedProperties: {
				// 出参结果数据列表
				sampleWaterSurfaceList: []
			}
		},
		mounted() {
			console.log('sampleWaterSurfaceInfo');
			this.init();
		},
		methods: {
			init() {
				if (this.extendedProperties.itemInfoList) {
					this.$set(this.extendedProperties, 'sampleWaterSurfaceList', this.extendedProperties.itemInfoList);
				}
			},
			refill() {
			},
			...
			submit() {
				if (this.enterBatchContent != '') {
					this.selectedRowKeys.forEach((data) => {
						this.extendedProperties.sampleWaterSurfaceList[data][this.enterBatchColumn] = this.enterBatchContent;
					});
				}
				this.cancel();
			},
