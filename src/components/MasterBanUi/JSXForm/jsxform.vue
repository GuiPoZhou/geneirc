                    ...
                    <div class="removearea">
                        ...
                        >
                            <div class="buttonArea">
                                <el-button type="text" style="color:red" size="mini" onClick={() => this.deleteExtItem(index)}>删除</el-button>
                    ...
                    <el-col key={index} span={params.col}>
                        <el-form-item label={params.label} prop={params['prop']} rules={params.rules}>
                            <el-select disabled={params.disabled} v-model={this.form[params['prop']]}
                                ...
                                onChange={(e) => { this.el_selectChange(e) }}
                            >
                                {
                    ...
                    return (
                        <div>
                            ...
                            <el-button type="text" size="small" onClick={() => {
                                this.e_showUploadFile(params.eventsList.showUploadFile, prop.file)
                            }}>下载
                            </el-button>
                            <el-button style="color:red" type="text" size="small" onClick={() => {
                                this.e_removeUploadFile(params.eventsList.removeUploadFile, prop.file, params)
                            }}>删除
                            </el-button>
                        </div>
                    )
                ...
                return (
                    <el-col key={index} span={params.col}>
                        <el-form-item label={params.label}>
                            <el-upload action={params.options.action}
                                disabled={params.disabled}
                                headers={params.options.headers}
                                scopedSlots={scopedSlots}
                                beforeRemove={(file) => {
                                    this.filebeforeRemove(params.eventsList.beforeRemove, file)
                                }}
                                before-upload={(file) => {
                                    this.fileBeforeUpload(params.eventsList.beforeUpload, file)
                                }}
                                // 这块需要尤其注意  在upload中 on-事件 是以属性来定义的
                                {...{
                                    props: {
                                        "on-success": (res, file) => {
                                            this.fileUploadOnSuccess(params.eventsList.onSuccess, res, file)
                                        },
                                        "on-remove": (file, fileList) => {
                                            this.fileOnRemove(params.eventsList.onRremove, file, fileList)
                                        },
                                        "on-preview": (file) => {
                                            this.fielOnPreview(params.eventsList.onPreview, file)
                                        }
                                    }
                                }}
                                multiple={true}
                                showFileList={true}
                                file-list={params.options.fujianList}
                            >
