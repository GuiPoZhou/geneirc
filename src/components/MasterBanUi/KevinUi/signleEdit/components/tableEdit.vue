                        ...
                        <el-row>
                            <el-col :span="12">
                                ...
                                <el-button size="mini" style="margin: 5px 0;" type="primary"
                                    @click="e_addMainTableActionButton">新增主表操作列按钮</el-button>
                                ...
                                <el-table :data="signleParams.tableColumns" style="width: 100%" height="400">
                                    <el-table-column prop="label" label="列名">
                                    </el-table-column>
                                    <el-table-column prop="prop" label="参数key">
                                    </el-table-column>
                                    <el-table-column prop="valueType" label="参数类型">
                                    </el-table-column>
                                    <el-table-column prop="valueType" label="参数类型">
                                    </el-table-column>
                                    <el-table-column prop="editType" label="渲染类型">
                                    </el-table-column>
                                    <el-table-column prop="slotRenderEvents" label="事件" show-overflow-tooltip>
                                    </el-table-column>
                                    <el-table-column label="操作">
                                        <template slot-scope="scope">
                                            <el-button v-if="scope.row.slotRenderEvents" type="text"
                                                @click="e_editMainTableDataEvents(scope.row.slotRenderEvents, scope.$index)">编辑事件脚本</el-button>
                                        </template>
                                    </el-table-column>
                                </el-table>
                    ...
                    <el-tab-pane v-if="signleParams.haveChild" label="子表格配置" name="second">
                        <el-row>
                            <el-col :span="12">
                                ...
                                <el-button size="mini" style="margin: 5px 0;" type="primary"
                                    @click="e_addSubTableActionButton">新增子表操作列按钮</el-button>
                                <el-table :data="signleParams.childTableSlotButtons" style="width: 100%" height="400">
                                    <el-table-column prop="label" label="按钮名称">
                                    </el-table-column>
                                    <el-table-column prop="actionStatusEvents" label="按钮是否禁用脚本" show-overflow-tooltip>
                                        <template slot-scope="scope">
                                            <span
                                                :style="{ color: subTableScriptEditType == 'subStatus' && subTableScriptIndex == scope.$index ? '#409EFF' : '#000' }">{{
                                                    scope.row.actionStatusEvents }}</span>
                                        </template>
                                    </el-table-column>
                                    <el-table-column prop="events" label="按钮事件脚本" show-overflow-tooltip>
                                        <template slot-scope="scope">
                                            <span
                                                :style="{ color: subTableScriptEditType == 'subEvents' && subTableScriptIndex == scope.$index ? 'rebeccapurple' : '#000' }">{{
                                                    scope.row.events }}</span>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="操作">
                                        <template slot-scope="scope">
                                            <el-button type="text"
                                                @click="e_editSubStatusScript(scope.row.actionStatusEvents, scope.$index)">编辑状态脚本</el-button>
                                            <el-button style="color: rebeccapurple;" type="text"
                                                @click="e_editSubTableEvents(scope.row.events, scope.$index)">编辑事件脚本</el-button>
                                        </template>
                                    </el-table-column>
                                </el-table>
                            </el-col>
