...
export default {
    ...
    methods: {
                ...
                return (
                    <div class={['removearea',params.isHide?'isHideSignle':'']}>
                        <div class="se_enter_info">
                            <span class="s_e_i_tip copybox" data-clipboard-text={params.type == 'fixed'?`${params.prop}`:`extData.${params.prop}`} onClick={()=>{this.e_copyParams()}}>{params.type == 'fixed'?`${params.prop}`:`extData.${params.prop}`}</span>
                        </div>
                        <div class="rm-btn-area">
                            <div class="rba-fun-icon" onClick={() => this.e_signleEdit(params)}>
                                <i class="el-icon-edit"></i>
                            ...
                            {
                                params.isHide ? <div class="rba-fun-icon" onClick={() =>{params.isHide = false;this.$forceUpdate()} }>
                                <i class="el-icon-view"></i>
                            </div>: <div class="rba-fun-icon" onClick={() => {params.isHide = true;this.$forceUpdate()}}>
                ...
                return (
                    <el-col key={index} span={params.col}>
                        ...
                        <el-form-item label={params.label}>
                            <el-upload action={this.$BASE_API + params.options.action + '?Authorization=' + getToken() + '&MenuId=' + localStorage.getItem('menuId')}
        ...
        renderELTreeTable(item,index){
            ...
            return (
                <el-col span={item.col}>
                    ...
                    <kevinTreeTable  tree-props={item.treeProps} row-key={item.rowKey} ref={item.anchorPoint} setColumn={true} key={item.anchorPoint}
        ...
        renderTableSelect(columnInfo, scope) {
            return (
                columnInfo.valueType == 'fixed' ?
                    <el-select disabled={columnInfo.disabled} v-model={scope.row[columnInfo.prop]} placeholder="请选择" onChange={(e) => {
                        columnInfo.events && columnInfo.events.change ? eval(columnInfo.events.change) : ''
                    }}>
                        {
                    ...
                    </el-select> : <el-select disabled={columnInfo.disabled}  v-model={scope.row.extData[columnInfo.prop]} placeholder="请选择" onChange={(e) => {
                        columnInfo.events && columnInfo.events.change ? eval(columnInfo.events.change) : ''
                    }}>
                        {
                            columnInfo.options.options.map(optioninfo => {
        ...
        */
        ...
        renderELTableChild(item, columnInfo, parentScope) {
            ...
            return (
                <kevinTable  ref={item.anchorPoint}  key={item.anchorPoint} scopedSlots={scopedSlots} columns={item.childTableColums}
                        ...
                        <el-row>
                            {
                                this.env != 'detail' ? (
                                    <el-col span={24}>
                                        <el-button type="primary" size="mini" onClick={() => { this.e_openKevinSign(params) }}>开始签名</el-button>
                                        <el-button type="danger" size="mini" onClick={() => { this.e_deleteKevinSign(params) }}>清除签名</el-button>
                                    </el-col>
                                ) : ''
                            }
                    ...
                    <el-form-item label={params.label} prop={params['prop']} rules={params.rules}>
                        <el-row>
                                ...
                                }} />
                            {
                                this.env != 'detail' ? (
                                    <el-col span={24}>
                                        <el-button type="primary" size="mini" onClick={() => { this.e_diyUploadImage() }}>上传图片</el-button>
                                        <el-button type="primary" size="mini" onClick={() => { this.e_openPhotogray(params) }}>高拍仪</el-button>
                                    </el-col>
                                ) : ''
                            }
                                                ...
                                                <div class="ki-list">
                                                    ...
                                                    <el-row>
                                                        <el-col span={24}>
                                                            ...
                                                            {
                                                                this.env != 'detail' ? (<i class="el-icon-delete  kil-icon ki-delete" onClick={() => { this.deleteKevinImage(params.options.fileList, fileIndex) }}></i>) : ''
                                                            }
        ...
        renderELTabs(params, index) {
            ...
            return (
                <el-col span={params.col}>
                    ...
                    <el-tabs v-model={params.activeName} type={params.type ? params.type : '-'} v-on:tab-click={()=>{params.events?eval(params.events):''}}>
                        {
