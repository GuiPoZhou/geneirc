...
export default {
    ...
    methods: {
        ...
        e_openKevinSign(params){
            this.diyKevinSignParams = params
            this.showsignature = true
            this.$nextTick(()=>{
                this.$refs.KevinSign.initWebSocket()
            })
        },
        // 回填签字屏内容
        e_saveSignImage(params){
            this.showsignature = false
            console.log('签字信息',params)
            this.diyKevinSignParams.options.fileList=[{showUrl:params.signimg,signTime:params.signTime}]
        },
        //打开高拍仪
    ...
    render(createElement, context) {
        return (
            <div class="kevinFormContainer">
                        ...
                        this.bolckFormParams.map((tabInfo, tabIndex) => {
                            ...
                            return (
                                <el-tab-pane label={tabInfo.tabTitle ? tabInfo.tabTitle : '看看自己写title了吗'}
                                    ...
                                    {
                                        this.microStore.state.system.closeDrag ? '' :
                                        <el-tooltip class="item" effect="light" content="管理维护该页签全局内自动执行脚本与手动执行脚本" placement="top-start"><el-button type="success" size="small" style="margin-bottom:10px"
                                                       onClick={() => {
                                    ...
                                    <draggable list={tabInfo.formList} animation={340}
                                               disabled={this.microStore.state.system.closeDrag} group={{
                                        name: `Tab${tabIndex}`,
                                        pull: false,
                                        put: false
                                    }}
                                            ...
                                            tabInfo.formList.map((blockInfo, blockIndex) => {
                                                if (!blockInfo.isHide) {
                                                    return (
                                                        ...
                                                        <div class={this.microStore.state.system.closeDrag ? '' :'blockClass'}>
                                                            {
                                                                this.microStore.state.system.closeDrag ? '' : <el-row>
                                                                    <el-col span={24}>
                ...
                {
                    this.showKevinLog ?
                        <kevinLog ref="kevinLog" open-env={this.openEnv} businessConfigId={this.businessConfigId} widgetForm={this.dialogWidget}
                                  onClose={() => {
                                      this.showKevinLog = false
                                  }} onSaveAddForm={() => {
                            this.e_saveWidgetForm()
                        }} onSaveBulletBoxScript={(params)=>{this.saveBulletBoxScript(params)}}     onSaveSignleEdit={(info)=>{this.e_saveSignleEdit}}/> : ''
                }
                {
                ...
                {
                    this.showAddWidget ?
                        <AddWidget editFor={this.editFor} businessConfigId={this.businessConfigId} ref="AddWidget" mainTableTags="ENTRUSTMENT_AGREEMENT"
                                   onSaveAddForm={() => {
                                       this.e_saveWidgetForm()
                                   }} onClose={() => {
                            this.showAddWidget = false
                ...
                {
                    this.showPhotographyVue ?<photographyVue ref="photographyVue" onClose={()=>{this.showPhotographyVue=false}} onImgBaseList={(list)=>{this.appendGpImage(list)}}/>:''
                }
                {
                    this.showScriptEditor ? <scriptEditor ref="scriptEditor" onClose={()=>{this.showScriptEditor=false}} onSave={(params)=>{this.saveScriptEditor(params)}}/>:''
                }
                {
                ...
                {
                    this.showsignature?<signature ref="KevinSign" onClose={()=>{this.showsignature=false}} onImgBase64={(signimg)=>{
                        this.e_saveSignImage(signimg)
                        }}/>:''
                }
            </div>
        )
