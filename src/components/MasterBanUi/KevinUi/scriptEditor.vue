                ...
                <div class="sa-left">
                    <el-tabs v-model="activeName" @tab-click="handleClick">
                        <el-tab-pane label="自动执行脚本(events)" name="events">
                            <el-button style="margin-bottom: 4px;" size="mini" type="primary" @click="e_addAutoRun">新增自执行脚本</el-button>
                            <el-collapse v-model="activeNames" @change="handleCollapseChange">
                                <el-collapse-item v-for="(value, key) in events" :key="key" :title="key" :name="key">
                        ...
                        <el-tab-pane label="手动执行脚本(passiveEvents)" name="passiveEvents">
                            <el-button style="margin-bottom: 4px;" size="mini" type="primary" @click="e_addPassiveEvents">新增手动执行脚本</el-button>

                            <el-collapse v-model="activeNames" @change="handleCollapseChange">
                                <el-collapse-item v-for="(value, key) in passiveEvents" :key="key" :title="key" :name="key">
                                    <span class="code-card__body" @click="e_clickCurrentCode('passiveEvents', value, key)">
                                        {{ value }}</span>
                                </el-collapse-item>
