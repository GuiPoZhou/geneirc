<template>
  <div class="HTable" style="margin-top:10px">
    <el-table width="100%" v-bind="$attrs" :data="tableData" v-on="$listeners" :key="JSON.stringify(checkedCol)"
      ...
      <el-table-column v-for="(item, index) in checkedCol" :key="item.prop" :selectable="checkSelectable" v-bind="item"
        :index="index" :column-key="item.prop" :align="item.align ? item.align : 'center'">
