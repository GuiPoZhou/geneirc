<template>
    <div class="MLTable">
      <!-- <div class="settingBox" v-if="setColumn">
        <el-popover placement="bottom-end" trigger="click" popper-class="settingPopper">
          <el-checkbox-group v-model="selectCol" @change="handleChangeSelectColumn">
            <el-checkbox v-for="item in col" :key="item.prop" :label="item.prop" :disabled="item.disabled"
              style="display:block;line-height:2;margin-right:0;">{{ item.label }}</el-checkbox>
          </el-checkbox-group>
          <i class="icon el-icon-setting" slot="reference"></i>
        </el-popover>
      </div> -->
      <el-table v-bind="$attrs" :data="tableData" v-on="$listeners" :key="JSON.stringify(checkedCol)" @select="e_select" @select-all="e_selctAll"  @selection-change="handleSelectionChange">
        <el-table-column v-for="(item, index) in checkedCol" :key="item.prop" v-bind="item" :index="index"
          ...
          :show-overflow-tooltip="item.showOverFlowToolTip ? item.showOverFlowToolTip : false" :type="item.type?item.type:''">
