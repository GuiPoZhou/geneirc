...
export default {
  ...
  computed: {
    ...
    actionFormat() {
      return this.config.uploadImage ? window.globalEnv.VUE_APP_BASE_API + this.config.uploadImage : window.globalEnv.VUE_APP_BASE_API + this.record.options.action;
    }
  },
  methods: {
	getContainer() {
    ...
    handleChange(info) {
      ...
      if (info.file.status === "done") {
        ...
        if (res.code === 200) {
          ...
          this.fileList.pop();
          this.$message.error(`图片上传失败 error:` + res.code);
        }
      } else if (info.file.status === "error") {
        this.$message.error(`图片上传失败 error:` + info.file.status);
      }
    }
  }
};
