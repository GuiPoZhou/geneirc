<template>
  <div class="a-c-top" style="padding-top: 0">
    <p class="title">设备分类</p>
    <el-tree
        :style="`height: ${580}px;overflow-y: scroll`"
        ref="tree"
        :props="defaultProps"
    ...
    <div class="btnbox" v-if="type === 'add'">
      <el-button icon="el-icon-delete" size="mini" :disabled="isdisable" @click="handleDel">删除</el-button>
      <el-button icon="el-icon-edit" size="mini" :disabled="isdisable" @click="handleEdit">修改</el-button>
      <el-button icon="el-icon-plus" size="mini" @click="addTree" :disabled="adddisable">增加</el-button>
    </div>
    <!--增加修改弹窗-->
    <el-dialog
        :title="diaTit"
...
<script>
  import { getDevtreeList, addDevtree, editDevtreeList, deleteDevtree } from '@/api/resources/deviceManagement/index.js'
  ...
  export default {
    ...
    methods: {
      // 获取书列表
      getTreelist () {
        getDevtreeList().then(res => {
          this.$emit('getTreedata', res.data)
          this.treedata = this.handleTree(res.data, "deptId");
          if (this.treedata.length === 0) {
            this.adddisable = false
          } else {
            this.adddisable = true
        ...
        this.$refs.queryForm.validate((valid) => {
          if (valid) {
            ...
            if (this.diaTit === '新增') {
              ...
              editDevtreeList(params).then(res => {
                if (res.code === 200) {
                  this.isloading = false
      ...
      handleDel () {
        this.$confirm("确认删除吗?", "提示", {
          ...
          deleteDevtree(this.choiceData.id).then(res => {
            if (res.code === 200) {
      ...
      handleEdit () {
        this.dialogVisible = true
        this.diaTit = '修改'
        this.queryParams.name = this.choiceData.nodeName
