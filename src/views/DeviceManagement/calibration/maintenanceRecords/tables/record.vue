    ...
    >
      ...
      >
        ...
        <el-table-column
            ...
            width="50"
        />
        <el-table-column prop="equipmentNumber" label="设备编号" align="center" sortable="custom" show-overflow-tooltip v-if="colData[0].istrue"/>
        <el-table-column prop="equipmentName" label="设备名称" align="center" sortable="custom" show-overflow-tooltip v-if="colData[1].istrue"/>
        <el-table-column prop="equipmentModel" label="规格型号" align="center" sortable="custom" show-overflow-tooltip v-if="colData[2].istrue"/>
        <el-table-column prop="planDate" label="计划实施日期" align="center" sortable="custom" show-overflow-tooltip v-if="colData[3].istrue"/>
        <el-table-column prop="calibrationDate" label="检定日期" align="center" sortable="custom" show-overflow-tooltip v-if="colData[4].istrue"/>
        <el-table-column prop="correct" label="修正值/修正因子" align="center" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue"/>
        <el-table-column label="检定校准结论" align="center" show-overflow-tooltip v-if="colData[6].istrue">
          <template slot-scope="scope">
            <span>
              {{scope.row.comfirmConclusion === 1 ? "符合" : scope.row.comfirmConclusion === 2 ? "不符合" : ""}}
              {{scope.row.useMode === 1 ? "正常使用" : scope.row.useMode === 2 ? "按修正值使用" : scope.row.useMode === 3 ? "不可正常使用" : ""}}
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="审批状态" align="center" sortable="custom" show-overflow-tooltip v-if="colData[7].istrue">
          <template slot-scope="scope">
...
<script>
  import { getCaliRecordList, deleteCaliRecord, CaliRecordSubmit } from '@/api/resources/deviceManagement/index.js'
  export default {
    name: "index",
    ...
    data () {
      return {
        ...
        colData: [
          ...
          { title: "检定日期", istrue: true },
          { title: "修正值/修正因子", istrue: true },
          { title: "检定校准结论", istrue: true },
    ...
    methods: {
      ...
      getList () {
        getCaliRecordList(this.queryParams).then(res => {
      ...
      handleDel() {
        this.$confirm("确定删除吗?", "提示", {
          ...
          deleteCaliRecord({idList: this.ids}).then(res => {
            if (res.code == 200) {
      ...
      handleSubmit () {
        ...
        this.$confirm("确定提交吗?", "提示", {
          ...
          CaliRecordSubmit({idList: idList}).then(res => {
            if (res.code == 200) {
		...
		handlePrint(){
			POBrowser.openWindowModeless(
				...
				"/equipment/pageOffice/printCalibrationRecord?ids=" + this.ids.join(","),
