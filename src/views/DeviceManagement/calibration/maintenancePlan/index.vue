<template>
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <el-table-column
            ...
            width="50"
        />
        <el-table-column prop="planDate" label="计划日期" sortable="custom" show-overflow-tooltip v-if="colData[0].istrue" align="center"/>
        <el-table-column prop="planNo" label="计划单号" sortable="custom" show-overflow-tooltip v-if="colData[1].istrue" align="center"/>
        <el-table-column prop="planYear" label="计划年度" sortable="custom" show-overflow-tooltip v-if="colData[2].istrue" align="center"/>
        <el-table-column prop="applyPerson" label="编制人员" sortable="custom" show-overflow-tooltip v-if="colData[3].istrue" align="center"/>
        <el-table-column prop="approvalUser" label="批准人" sortable="custom" show-overflow-tooltip v-if="colData[4].istrue" align="center"/>
        <el-table-column prop="approvalDate" label="批准日期" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue" align="center"/>
        <el-table-column prop="describe" label="计划描述" show-overflow-tooltip v-if="colData[6].istrue" align="center"/>
        <el-table-column prop="status" label="计划状态" sortable="custom" show-overflow-tooltip v-if="colData[7].istrue" align="center">
    ...
    <!--添加/修改-->
    <createPlan :deviceK18Info="deviceK18Info" v-if="showplan" ref="plan" @close="showplan = false"></createPlan>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getList () {
        getCalibrationPlanList(this.queryParams).then(res => {
      ...
      getDetails (id, type) {
        this.showplan = true
        getCalibrationPlanDetail(id).then(res => {
          this.$nextTick(() => {
      ...
      e_del (id) {
        this.$confirm('确定删除该条数据吗?', "提示", {
          ...
          deleteCalibrationPlan(id).then(res => {
            if (res.code === 200) {
      ...
      handleSubmit () {
        ...
        this.$confirm("确定提交吗?", "提示", {
          ...
          submitCalibrationPlan({idList: idList}).then(res => {
            if (res.code == 200) {
		...
		handlePrint(){
			POBrowser.openWindowModeless(
				...
				"/equipment/pageOffice/printCalibrationPlan?id=" + this.ids[0],
