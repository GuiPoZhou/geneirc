  ...
  <el-dialog :title="titles" :visible.sync="dialogShow" center :show-close="true" :close-on-press-escape="false"
    ...
    <el-form :model="queryParams" ref="queryForm" :prop="queryParams" :rules="rules" label-width="120px">
      <el-tabs v-model="activeName" :stretch="true">
        <el-tab-pane label="检定校准计划" name="first">
          <el-row>
            <el-col :span="8">
          ...
          <el-table ref="multipleTable" :data="tableData" stripe border style="margin-top: 20px"
            ...
            <el-table-column prop="factoryNumber" label="出厂编号" show-overflow-tooltip />
            <el-table-column prop="calibrationMode" label="计划朔源方式" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{ scope.row.calibrationMode === '1' ? '检定' : scope.row.calibrationMode === '2' ? '校准' :
                  scope.row.calibrationMode === '3' ? '期间核查' : '' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="traceCompany" :label="deviceK18Info.code == 500 ? '计划溯源单位' : deviceK18Info.traceCompany"
              show-overflow-tooltip>
              <template slot-scope="scope">
                <el-input v-model="scope.row.traceCompany" :disabled="isdisable"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="lastCalibrationDate" label="上次检定校准日期" show-overflow-tooltip />
    ...
    <!--选择设备-->
    <addEquipment v-if="showequiment" :typeofs="typeofs" ref="equiment" @close="showequiment = false"
...
import { addCalibrationPlan, editCalibrationPlan } from '@/api/resources/deviceManagement/index'
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],
      multipleSelection: [],
      typeofs: 'EQUIPMENT_CALIBRATION_PLAN',
      showequiment: false,
  ...
  methods: {
    ...
    getProcess() {
      const params = {
        businessType: 'EQUIPMENT_CALIBRATION_PLAN',
      ...
      this.$refs.queryForm.validate((valid) => {
        if (valid) {
          ...
          if (this.queryParams.id) {
            editCalibrationPlan(this.queryParams).then(res => {
              if (res.code === 200) {
            ...
            addCalibrationPlan(this.queryParams).then(res => {
              if (res.code === 200) {
