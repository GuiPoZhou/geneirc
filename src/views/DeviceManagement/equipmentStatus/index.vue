<template>
  <div class="app-container">
    ...
    <div class="a-c-top" v-show="showSearch">
      ...
      >
        ...
        <el-form-item label="使用部门" prop="userDepartmentName">
          <el-input
            v-model="queryParams.userDepartmentName"
            placeholder="请输入部门"
            clearable
            class="input-with-select"
            style="width: 240px"
        ...
        <el-col :span="20" :xs="24">
          <div :class="fullscreen == true ? 'showFullScreen' : ''"
            ...
            <el-row :gutter="10" class="searchbox">
              <!--              <span style="font-size: 14px; font-weight: bold;color: #333;padding-top: 5px;line-height: 20px">设备状态</span>-->
            ...
            >
              ...
              <el-table-column prop="name" label="操作" width="100px" align="center">
                <template slot-scope="scope">
                  <el-button size="mini" type="text" @click="e_edit(scope.row)">修改</el-button>
                  <el-button size="mini" type="text" @click="e_history(scope.row)">历史</el-button>
                </template>
              </el-table-column>
            </el-table>
            <pagination
    ...
    <!--新增/修改-->
    <editStatus v-if="showstatus" ref="state" @close="showstatus = false"></editStatus>
    <!--历史-->
    <editHistory v-if="showhistory" ref="history" @close="showhistory = false"></editHistory>
  </div>
</template>

<script>
  import catalogueTree from '../components/catalogueTree'
  import editStatus from './dialog/editStatus'
  import editHistory from './dialog/editHistory'
  import {getEquipmentStatusList, getEquipmentStatusHis} from '@/api/resources/deviceManagement/index.js'

  export default {
    name: 'index',
    components: {catalogueTree, editStatus, editHistory},
    data() {
      return {
        showSearch: true,
        queryParams: {
    ...
    methods: {
      ...
      // 获取列表
      getList() {
        getEquipmentStatusList(this.queryParams).then(res => {
      ...
      e_edit(row) {
        ...
        this.$nextTick(() => {
          this.$refs.state.init('update', row)
        })
      },
      // 历史
      e_history(row) {
        this.showhistory = true
        getEquipmentStatusHis({equipmentId: row.id}).then(res => {
          this.$nextTick(() => {
            this.$refs.history.init(res.data)
          })
        })
      }
    }
