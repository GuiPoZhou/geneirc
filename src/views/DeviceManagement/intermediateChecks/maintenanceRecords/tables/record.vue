    ...
    >
      ...
      >
        ...
        <el-table-column
            ...
            width="50"
        />
        <el-table-column prop="equipmentNumber" label="设备编号" sortable="custom" show-overflow-tooltip v-if="colData[0].istrue" align="center"/>
        <el-table-column prop="equipmentName" label="设备名称" sortable="custom" show-overflow-tooltip v-if="colData[1].istrue" align="center"/>
        <el-table-column prop="equipmentModel" label="规格型号" sortable="custom" show-overflow-tooltip v-if="colData[2].istrue" align="center"/>
        <el-table-column prop="planDate" label="计划实施日期" sortable="custom" show-overflow-tooltip v-if="colData[3].istrue" align="center"/>
        <el-table-column prop="checkDate" label="核查日期" sortable="custom" show-overflow-tooltip v-if="colData[4].istrue" align="center"/>
<!--        <el-table-column prop="approvalUser" label="核查部门" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue"/>-->
        <el-table-column prop="checkPerson" label="核查人员" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue" align="center"/>
        <el-table-column prop="status" label="审批状态" sortable="custom" show-overflow-tooltip v-if="colData[6].istrue" align="center">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="text" @click="e_del(scope.row.id)" v-if="scope.row.status === 1 || scope.row.status === 4" style="color: #409EFF">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination
...
<script>
  import { getrecordCheckList, deleteCheckRecord, checksubmitRecord } from '@/api/resources/deviceManagement/index.js'
  export default {
    name: "index",
    ...
    data () {
      return {
        ...
        colData: [
          ...
          { title: "核查日期", istrue: true },
          // { title: "核查部门", istrue: true },
          { title: "核查人员", istrue: true },
    ...
    methods: {
      ...
      getList () {
        getrecordCheckList(this.queryParams).then(res => {
      ...
      handleDel() {
        this.$confirm("确定删除吗?", "提示", {
          ...
          deleteCheckRecord({idList: this.ids}).then(res => {
            if (res.code == 200) {
      ...
      handleSubmit () {
        ...
        this.$confirm("确定提交吗?", "提示", {
          ...
          checksubmitRecord({idList: idList}).then(res => {
            if (res.code == 200) {
		...
		handlePrint(){
			POBrowser.openWindowModeless(
				...
				"/equipment/pageOffice/printCheckRecord?ids=" + this.ids.join(","),
