		...
		>
			...
			>
				...
				<el-table-column
					...
					width="50"
				/>
				<el-table-column prop="planDate" label="计划日期" sortable="custom" show-overflow-tooltip align="center"
								/>
				<el-table-column prop="planNo" label="计划单号" sortable="custom" show-overflow-tooltip align="center"
								/>
				<el-table-column prop="planYear" label="计划年度" sortable="custom" show-overflow-tooltip align="center"
								 />
				<el-table-column prop="applyPerson" label="编制人员" sortable="custom" show-overflow-tooltip align="center"
								 />
				...
				<el-table-column prop="approvalDate" label="批准日期" sortable="custom" show-overflow-tooltip align="center"
								 />
				<el-table-column prop="describe" label="计划描述" show-overflow-tooltip  align="center"/>
		
				<el-table-column prop="status" label="计划状态" sortable="custom" show-overflow-tooltip align="center"
								 >
				...
				<el-table-column label="操作" align="center" width="200" fixed="right">
					<template slot-scope="scope">
						...
						<el-button size="mini" type="text" v-if="scope.row.status === 1 || scope.row.status === 4"
								   @click="e_edit(scope.row.id)" style="color: #409EFF">修改
						</el-button>
						<el-button size="mini" type="text"
								   v-if="scope.row.status === 1 || scope.row.status === 4" @click="e_del(scope.row.id)" style="color: #409EFF">
							删除
						</el-button>
...
<script>
    ...
    async  created() {
      this.projectSlotButtonList = await this.getProjectSlotButtons('maintenancePlanButton')
    },
		mounted() {
			this.getList()
		},
      ...
      checkActionButton(events, scope) {
        return eval(events)
			...
			getList() {
				getMaintainPlanList(this.queryParams).then(res => {
			...
			getDetails(id, type) {
				this.showplan = true
				getEquipmentplanDetail(id).then(res => {
					this.$nextTick(() => {
			...
			e_del(id) {
				this.$confirm('确定删除该条数据吗?', '提示', {
					...
					deleteMaintainPlan(id).then(res => {
						if (res.code === 200) {
			...
			handleSubmit() {
				...
				this.$confirm('确定提交吗?', '提示', {
					...
					submitMaintainPlan({ idList: idList }).then(res => {
						if (res.code == 200) {
			...
			handlePrint() {
				POBrowser.openWindowModeless(
					...
					'/equipment/pageOffice/printMaintainPlan?id=' + this.ids[0],
