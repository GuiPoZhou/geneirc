<template>
    ...
    >
      <el-tabs v-model="activeName" :stretch="true">
        <el-tab-pane label="维护保养计划" name="first">
          <el-row>
            <el-col :span="8">
              <el-form-item label="计划日期" prop="planDate">
                <el-date-picker v-model="queryParams.planDate" value-format="yyyy-MM-dd" type="date"
          ...
          >
            ...
            <el-table-column prop="maintainCycle" label="保养周期" show-overflow-tooltip>
              <template slot-scope="scope">
                <el-input v-model="scope.row.maintainCycle" :disabled="isdisable"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="maintainContent" label="维护保养内容" show-overflow-tooltip>
              <template slot-scope="scope">
                <el-input v-model="scope.row.maintainContent" :disabled="isdisable"></el-input>
              </template>
            </el-table-column>
...
<script>
  ...
  import { addEquipmentplan, editEquipmentPlan } from '@/api/resources/deviceManagement/index'
  ...
  export default {
    ...
    data() {
      return {
        ...
        typeofs: 'EQUIPMENT_MAINTAIN_PLAN',
        approvalProcess: {},
      }
    },
    computed: {
    ...
    methods: {
      ...
      getProcess() {
        const params = {
          businessType: 'EQUIPMENT_MAINTAIN_PLAN',
      ...
      handleSubmit (status) {
        this.$refs.queryForm.validate((valid) => {
          if (valid) {
            ...
            // if (this.queryParams.detailList.length === 0) {
            ...
            if (this.queryParams.id) {
              editEquipmentPlan(this.queryParams).then(res => {
                if (res.code === 200) {
              ...
              addEquipmentplan(this.queryParams).then(res => {
                if (res.code === 200) {
