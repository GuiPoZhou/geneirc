  ...
  <div style="height: 85vh">
    ...
    <div class="a-c-top" v-show="showSearch">
      ...
      <el-form :inline="true" :model="queryParams" ref="queryParams" class="demo-form-inline" v-show="showSearch"
        ...
        <el-form-item label="使用班组" prop="userDepartment">
          <treeselect style="width: 240px" size="small" v-model="queryParams.userDepartment" :options="deptOptions"
        ...
        <el-col :span="20" :xs="24">
          <div :class="fullscreen == true ? 'showFullScreen' : ''" style="padding-bottom:4px; padding-right: 5px">
            ...
            <el-table ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange"
              ...
              <el-table-column prop="maintainCycle" label="保养周期" sortable="custom" show-overflow-tooltip align="center"
                v-if="colData[6].istrue">
              </el-table-column>
              <el-table-column prop="maintainContent" label="维护保养内容" sortable="custom" align="center"
                show-overflow-tooltip v-if="colData[7].istrue" />
              <el-table-column prop="planDates" label="计划实施日期" sortable="custom" show-overflow-tooltip align="center"
                v-if="colData[8].istrue" />
              <el-table-column prop="liablePerson" label="责任人" sortable="custom" show-overflow-tooltip align="center"
              ...
              <el-table-column prop="name" label="操作" align="center" width="120" fixed="right">
                <template slot-scope="scope">
                  <el-button size="mini" type="text" @click="e_implement(scope.row)" style="color: #409EFF">执行
                  </el-button>
                </template>
              </el-table-column>
            </el-table>
...
import { getWatingplanList } from '@/api/resources/deviceManagement/index.js'
...
export default {
  ...
  data() {
    return {
      ...
      colData: [
        ...
        { title: '保养周期', istrue: true },
        { title: '维护保养内容', istrue: true },
  ...
  methods: {
    ...
    // 获取列表
    getList() {
      getWatingplanList(this.queryParams).then(res => {
        for (let i = 0; i < res.data.list.length; i++) {
          res.data.list[i].planDates = res.data.list[i].planDate.split('-')[0] + '-' + res.data.list[i].planDate.split('-')[1]
        }
