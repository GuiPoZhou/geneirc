<template>
  <el-dialog title="维护上岗证范围" :visible.sync="showDialog" append-to-body width="800px"
    ...
    <!-- 表格 -->
    <div>
      <el-table height="500" :data="categoryList" ref="multipleTable" row-key
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取用户列表
      getList() {
        getDetectCategoryList(this.queryParams).then(response => {
      ...
      handleSelectionChange(select) {
        setTimeout(() => {
          this.selectTableData = this.removeRepeat([...select, ...this.selectTableData])
          if (this.selectTableData) {
            this.selectTableData.forEach(row => {
              this.categoryList.forEach((item) => {
                if (row.id == item.id) {
      ...
      commit() {
        ...
        this.$parent.$parent.backFillCategory(this.selectTableData)
        this.close()
        this.$nextTick(() => {
