<template>
  <div class="a-c-top">
    ...
    <div style="width: 90%;margin: 0 auto 10px" v-if="type !== '0'">
      <el-input
    ...
    <div class="btnbox" v-if="copy !== true">
...
<script>
  import {
    addproductStandardtree,
    deleteproductStandardtree,
    editproductStandardtree
    ...
    methods: {
        ...
        this.$refs.queryForm.validate((valid) => {
          if (valid) {
            ...
            if (this.diaTit === '新增') {
              ...
              addproductStandardtree(params).then(res => {
                if (res.code === 200) {
              ...
              editproductStandardtree(params).then(res => {
                if (res.code === 200) {
                  this.isloading = false
      ...
      handleDel() {
        this.$confirm("确认删除吗?", "提示", {
          ...
          deleteproductStandardtree(this.choiceData.id).then(res => {
            if (res.code === 200) {
      ...
      renderContent(h, { node, data, store }) {
        return(<span class={data.status === '1' ? 'dataStyle' : ''}>{node.data.name}</span>)
      },
    }
  }
</script>
