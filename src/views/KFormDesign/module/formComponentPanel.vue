        ...
        <transition-group tag="div" name="list" class="list-main">
          <layoutItem
            ...
            :insertFormulaAllowedType="insertFormulaAllowedType"
						:insertSharedFormAllowedType="insertSharedFormAllowedType"
            :hideModel="hideModel"
            @dragStart="dragStart"
...
import "@/components/codemirror/mode/javascript/javascript";
...
export default {
  ...
  methods: {
    ...
    handleColAdd(evt, columns, isCopy = false) {
      ...
      if (columns[newIndex].key === "" || isCopy) {
        ...
        if (typeof columns[newIndex].list !== "undefined") {
          // 深拷贝list
          if (columns[newIndex].type === 'plan') {
            columns[newIndex].list = basicsList[basicsList.length - 1].list
          } else if (columns[newIndex].type === 'dynamicFormula'){
            const listStr = JSON.stringify(columns[newIndex].list);
            columns[newIndex].list = JSON.parse(listStr);
          } else {
      ...
      const traverse = array => {
        array.forEach((element, index) => {
          ...
          if (element.type === "grid") {
            ...
            if (!isCopy && !this.insertFormulaAllowedType.includes(data.type)) {
              return false;
            }
            traverse(element.list);
					} else if (element.type === "sharedForm") {
						if (!isCopy && !this.insertSharedFormAllowedType.includes(data.type)) {
							return false;
						}
    ...
    handleDetele() {
      ...
      if(this.selectItem.options && this.selectItem.options.tableColumn){
        this.$message.error("系统默认属性,不允许删除!");
        return;
      }
      // 删除已选择
        const traverse = array => {
        ...
        array = array.filter((element, index) => {
          ...
          if (element.type === "card" || element.type === "batch" || element.type === "plan" ) {
            console.log(element);
            // 卡片布局
            element.list = traverse(element.list);
          }
          if(element.type === "dynamicFormula" || element.type === "brackets"){
            element.list = traverse(element.list);
          }
