  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <el-table-column prop="businessType" label="替换/更改" sortable="custom" show-overflow-tooltip v-if="colData[2].istrue" align="center">
          <template slot-scope="scope">
            <span v-if="scope.row.businessType === 1">替换</span>
            <span v-if="scope.row.businessType === 2">更改</span>
          </template>
        </el-table-column>
        <el-table-column prop="replaceDate" label="替换/更改日期" sortable="custom" show-overflow-tooltip v-if="colData[3].istrue" align="center"/>
        <el-table-column prop="remark" label="备注" sortable="custom" show-overflow-tooltip v-if="colData[4].istrue" align="center"/>
...
<script>
  ...
  import {
    getReplaceRecordList,
    getReplaceRecorddetail,
    deleteReplaceRecord,
    exportReplaceRecord
  ...
  export default {
    ...
    methods: {
      ...
      getList () {
        getReplaceRecordList(this.queryParams).then(res => {
      ...
      e_edit (row) {
        this.showFile = true
        getReplaceRecorddetail(row.id).then(res => {
          this.$nextTick(() => {
      ...
      e_del (row) {
        this.$confirm("确定删除吗?", "提示", {
          ...
          deleteReplaceRecord(row.id).then(res => {
            if (res.code == 200) {
      ...
      handleExport() {
        ...
        exportReplaceRecord(this.queryParams).then(res => {
          if (res.code === 200) {
		...
		handlePrint(){
			...
			POBrowser.openWindowModeless(
				...
				"/fileManage/pageOffice/printReplace?ids=" + array.join(","),
