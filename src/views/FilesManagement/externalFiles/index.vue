    ...
    >
      ...
      >
        ...
        <el-table-column
            ...
            fixed
        />
        <el-table-column prop="fileNumber" label="文件编号" sortable="custom" show-overflow-tooltip fixed
                         v-if="colData[0].istrue" align="center"/>
        <el-table-column prop="fileName" label="文件名称" sortable="custom" show-overflow-tooltip fixed
                         v-if="colData[1].istrue" align="center"/>
        <el-table-column prop="receiveData" label="引入日期" sortable="custom" show-overflow-tooltip
                         v-if="colData[2].istrue" align="center"/>
        <el-table-column prop="receiverName" label="接收者" sortable="custom" show-overflow-tooltip
                         v-if="colData[3].istrue" align="center"/>
        <el-table-column prop="keeperName" label="引入部门负责人" sortable="custom" show-overflow-tooltip
                         v-if="colData[4].istrue" align="center"/>
        <el-table-column prop="status" label="文件状态" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue"
                         align="center">
        ...
        <el-table-column prop="name" label="操作" align="center" width="200" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="text" @click="e_delete(scope.row)">
              删除
            </el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      colData: [
        ...
        {title: "引入日期", istrue: true},
        {title: "接收者", istrue: true},
        {title: "引入部门负责人", istrue: true},
        {title: "备注", istrue: true},
        // { title: "文件状态", istrue: true }
      ],
      tableData: [],
      multipleSelection: [],
      ids: []
  ...
  methods: {
    ...
    getList() {
      getexternalFileList(this.queryParams).then(res => {
    ...
    e_look(row) {
      this.showFile = true
      getExternaldetail(row.id).then(res => {
        this.$nextTick(() => {
    ...
    e_delete(row) {
      this.$confirm("确定删除吗?", "提示", {
        ...
        deleteExternalFile(row.id).then(res => {
          if (res.code == 200) {
    ...
    handleExport() {
      ...
      exportExternal(this.queryParams).then(res => {
        if (res.code === 200) {
    ...
    handlePrint() {
      ...
      POBrowser.openWindowModeless(
          ...
          "/fileManage/pageOffice/printExternal?ids=" + array.join(","),
