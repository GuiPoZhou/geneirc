    ...
    >
      ...
      >
        ...
        <el-table-column
            ...
            fixed
        />
        <el-table-column prop="fileNumber" label="文件编号" sortable="custom" show-overflow-tooltip fixed v-if="colData[0].istrue" align="center"/>
        <el-table-column prop="fileName" label="文件名称" sortable="custom" show-overflow-tooltip v-if="colData[1].istrue" align="center"/>
        <el-table-column prop="amount" label="份数" sortable="custom" show-overflow-tooltip v-if="colData[2].istrue" align="center"/>
        <el-table-column prop="secretLevel" label="保密等级" sortable="custom" show-overflow-tooltip v-if="colData[3].istrue" align="center"/>
        <el-table-column prop="archivesNumber" label="归档编号" sortable="custom" show-overflow-tooltip v-if="colData[4].istrue" align="center"/>
        <el-table-column prop="location" label="存放位置" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue" align="center"/>
        <el-table-column prop="keepingMonthStr" label="保管期限" sortable="custom" show-overflow-tooltip v-if="colData[6].istrue" align="center"/>
        <el-table-column prop="archivesDate" label="归档日期" sortable="custom" show-overflow-tooltip v-if="colData[7].istrue" align="center"/>
        <el-table-column prop="remark" label="备注" sortable="custom" show-overflow-tooltip v-if="colData[8].istrue" align="center"/>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        colData: [
          ...
          {title: "份数", istrue: true},
          {title: "保密等级", istrue: true},
          {title: "归档编号", istrue: true},
          {title: "存放位置", istrue: true},
          {title: "保管期限", istrue: true},
          {title: "归档日期", istrue: true},
    ...
    methods: {
      ...
      e_look(row) {
        this.showFile = true
        getArchivesdetail(row.id).then(res => {
          this.$nextTick(() => {
      ...
      e_del (row) {
        this.$confirm("确定删除吗?", "提示", {
          ...
          deleteArchives(row.id).then(res => {
            if (res.code == 200) {
      ...
      handleExport() {
        ...
        exportArchives(this.queryParams).then(res => {
          if (res.code === 200) {
      ...
      handlePrint() {
        ...
        POBrowser.openWindowModeless(
          ...
          '/fileManage/pageOffice/printArchives?ids=' + array.join(','),
