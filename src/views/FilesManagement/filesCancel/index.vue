  ...
  <div class="app-container">
    ...
    <div class="a-c-bottom" :class="fullscreen == true ? 'showFullScreen' : ''">
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <el-button type="primary" @click="handleAdd" size="mini">添加
          </el-button>
          <el-button type="primary" :disabled="!this.multipleSelection.length > 0" @click="handlePrint" size="mini">打印
          </el-button>
          <el-button type="warning" @click="handleExport" size="mini">导出
          </el-button>
        </el-col>
      ...
      <!-- 列表区域 -->
      <el-table ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @sort-change="sortChange">
        <el-table-column type="selection" width="55" fixed />
        <el-table-column label="序号" align="center" type="index" width="50" fixed />
        <el-table-column prop="fileName" label="文件名称" sortable="custom" show-overflow-tooltip v-if="colData[0].istrue"
          align="center" />
        <el-table-column prop="fileNumber" label="文件编号" sortable="custom" show-overflow-tooltip v-if="colData[1].istrue"
          align="center" />
        <el-table-column prop="abolishDate" label="作废日期" sortable="custom" show-overflow-tooltip v-if="colData[2].istrue"
          align="center" />
        <el-table-column prop="reason" label="作废原因" sortable="custom" show-overflow-tooltip v-if="colData[3].istrue"
          align="center" />
        <el-table-column prop="handler" label="申请人" sortable="custom" show-overflow-tooltip v-if="colData[4].istrue"
          align="center" />
        <el-table-column prop="approver" label="批准人" sortable="custom" show-overflow-tooltip v-if="colData[5].istrue"
          align="center" />
        <el-table-column prop="remark" label="备注" sortable="custom" show-overflow-tooltip v-if="colData[6].istrue"
          align="center" />
        <!-- <el-table-column prop="amount" label="作废份数" sortable="custom" show-overflow-tooltip v-if="colData[7].istrue"
          align="center" /> -->
  ...
  data() {
    return {
      ...
      colData: [
        ...
        { title: "作废日期", istrue: true },
        { title: "作废原因", istrue: true },
        { title: "申请人", istrue: true },
        { title: "批准人", istrue: true },
        { title: "备注", istrue: true },
        // { title: "作废份数", istrue: true }
  ...
  methods: {
    ...
    handleQuery() {
      if (this.dateValue === null) {
        this.queryParams.entrustDateStart = undefined;
        this.queryParams.entrustDateEnd = undefined;
      }
    ...
    getList() {
      getAbolishList(this.queryParams).then(res => {
    ...
    e_look(row) {
      this.showFile = true
      getAbolishdetail(row.id).then(res => {
        this.$nextTick(() => {
    ...
    handleExport() {
      ...
      exportAbolish(this.queryParams).then(res => {
        if (res.code === 200) {
    ...
    handlePrint() {
      ...
      POBrowser.openWindowModeless(
        ...
        '/fileManage/pageOffice/printAbolish?ids=' + array.join(','),
