<template>
    ...
    >
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="作废份数" prop="amount">
            <el-input placeholder="请输入作废份数" v-model="queryParams.amount" oninput="value = value.replace(/[^\d]/g,'')" :disabled="isdisable"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="作废日期" prop="abolishDate" style="width: 100%">
            <el-date-picker v-model="queryParams.abolishDate" value-format="yyyy-MM-dd" type="date"
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="办理人" prop="handler">
            <el-input placeholder="请输入办理人" v-model="queryParams.handler" disabled></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="批准人" prop="approver" style="width: 100%">
            <el-input placeholder="请输入批准人" v-model="queryParams.approver" :disabled="isdisable"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item label="是否销毁" prop="standardNameCn">
            <el-checkbox v-model="queryParams.checked" :disabled="isdisable">是</el-checkbox>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
...
<script>
  ...
  import { addAbolish, editAbolish, getInsertOperateRecord } from '@/api/filesManagement/filesManagement'
  export default {
    name: "craeteFile",
    data() {
      return {
    ...
    methods: {
      ...
      open() {
        if (this.queryParams.id) {
          console.log(this.queryParams.uploadFileList)
      ...
      e_getInfo() {
        getInfo().then(res => {
          this.queryParams.handler = res.user.nickName
        })
      },
      // 文件上传路径返回
      uploadUrl() {
        ...
        this.$refs.queryForm.validate((valid) => {
          if (valid) {
            ...
            if (this.queryParams.checked === true) {
              this.queryParams.abolishFlag = 1
            } else {
              this.queryParams.abolishFlag = 0
            }
            console.log(this.queryParams)
            this.isloading = true
            if (this.queryParams.id) {
              editAbolish(this.queryParams).then(res => {
                if (res.code === 200) {
              ...
              addAbolish(this.queryParams).then(res => {
                if (res.code === 200) {
