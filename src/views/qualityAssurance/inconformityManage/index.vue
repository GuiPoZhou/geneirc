...
import { nolist, nodetail, nodelete,exportIncompatible } from "@/api/mass/config";
import Treeselect from "@riophae/vue-treeselect";
import "@riophae/vue-treeselect/dist/vue-treeselect.css";
import createInconform from "./dialog/createInconform";
export default {
  name: "index",
  components: { Treeselect, createInconform },
  data() {
    return {
      showSearch: true,
      fullscreen: false,
      ...
      tableData: [],
      colData: [
        { title: "不符合记录编号", istrue: true },
        { title: "不符合简述", istrue: true },
        { title: "责任部门", istrue: true },
  ...
  methods: {
	  handleExport(){
		  exportIncompatible(this.queryParams).then(res => {
			  if (res.code === 200) {
    ...
    // 显示列表数据
    getList() {
      nolist(this.queryParams).then((res) => {
    ...
    handlePrint() {
      POBrowser.openWindowModeless(
        process.env.VUE_APP_BASE_API +
          "/qualityControl/pageOffice/printIncompatible?ids=" +
          this.ids.join(","),
