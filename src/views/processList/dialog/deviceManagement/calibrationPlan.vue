<template>
  <div style="padding: 20px">
    <el-tabs v-model="activeName" @tab-click="handleClick" type="card">
      <el-tab-pane label="检定校准计划申请" name="first">
        <el-form
            :model="queryParams"
            ref="queryForm"
            :inline="true"
        ...
        >
          ...
          <el-table-column prop="calibrationMode" label="计划检定/校准方式" show-overflow-tooltip>
            <template slot-scope="scope">
              <span>{{scope.row.calibrationMode === '1' ? '内检/内校' : scope.row.calibrationMode === '2' ? '外检/外校' : ''}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="traceCompany" label="计划溯源单位" show-overflow-tooltip/>
