...
<template>
  ...
  >
    ...
    <a-button type="dashed" :disabled="disabled" @click="addDomain" v-if="record.type === 'batch'">
      <a-icon type="plus" />增加
    </a-button>
    <customProject v-else-if="record.type === 'plan'" :record="record" :ref="customProjectData" :envDetectInfoPick="envDetectInfoPick" :extendedProperties="extendedProperties"/>
  </a-form-model>
</template>

<script>
import KFormModelItem from "./module/KFormModelItem";
import customProject from '@/components/project/customProject';
let id = 1;
export default {
  name: "KBatch",
  props: ["record", "value", "dynamicData", "config", "parentDisabled", "customProjectData", "envDetectInfoPick","extendedProperties"],
  components: {
    KFormModelItem,
    customProject,
  },
